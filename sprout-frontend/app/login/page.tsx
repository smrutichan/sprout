...
export default function Login() {
  ...
  return (
    <div className="h-screen lg:overflow-hidden overflow-y-auto flex flex-col lg:flex-row bg-gradient-to-br from-amber-50/60 via-emerald-50/40 to-teal-50/50 dark:from-zinc-950 dark:via-zinc-900/95 dark:to-zinc-950 font-sans transition-colors duration-500 relative">
      ...
      {/* RIGHT SIDE - COZY LOGIN CARD (≈42% width on desktop) */}
      <div className="w-full lg:w-[42%] flex-shrink-0 flex-grow-0 flex flex-col justify-center items-center p-6 md:p-8 lg:py-6 relative z-10 lg:overflow-hidden">
        ...
        >
          ...
          {/* Wholesome glassmorphic login card */}
          <div className="bg-white/70 dark:bg-zinc-900/70 border border-white/60 dark:border-zinc-800/80 backdrop-blur-2xl shadow-xl shadow-emerald-900/5 rounded-3xl p-5 sm:p-6 pt-7 transition-colors duration-300">
            ...
            <form onSubmit={handleLogin} className="space-y-3.5">
              ...
              <div className="flex items-center justify-between text-[11px] pt-0.5">
                <label className="flex items-center gap-1.5 font-bold text-slate-500 dark:text-zinc-400 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={rememberMe}
                    onChange={(e) => setRememberMe(e.target.checked)}
                    className="w-3.5 h-3.5 rounded border-slate-300 dark:border-zinc-700 text-emerald-500 focus:ring-emerald-500 bg-slate-50 dark:bg-zinc-950 cursor-pointer accent-emerald-500"
                  />
                ...
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    setError("Password reset is mock-only in this demo version.");
                  }}
                  className="font-extrabold text-emerald-600 dark:text-emerald-400 hover:underline hover:text-emerald-500 transition-colors"
                >
                  Forgot Password?
                </a>
              </div>

              {/* Submit / Nurture Button */}
              <motion.button
                type="submit"
                disabled={loading}
